...
require_once "database/db-connect.php";
		...
		$input_pkt_zrecznosc = $_GET['input-pkt-zrecznosc'];
		 
		 if(empty($input_pkt_zrecznosc)){
			 ...
			 exit();
						  ...
						  $sql="SELECT pkt, zrecznosc, respekt, energia FROM uzytkownicy WHERE id='$user_id'";
						  ...
						  $nowepkt = $row['pkt'] - $input_pkt_zrecznosc;
			 if($nowepkt<0){
			 ...
			 header ('Location: ../game-crims-city.php?link=character');
						  ...
						  $baza_pkt_zrecznosc = $row['zrecznosc'];
						  $respekt = $row['respekt'];
						  $nowepktzrecznosc = $row['zrecznosc'] + $input_pkt_zrecznosc;
						
						  $sql1="UPDATE `uzytkownicy` SET `pkt`= '$nowepkt' WHERE id='$user_id'";
						  $sql2="UPDATE `uzytkownicy` SET `zrecznosc`= '$nowepktzrecznosc' WHERE id='$user_id'";
						  $nowy_respekt = $respekt + (60 * $input_pkt_zrecznosc);
