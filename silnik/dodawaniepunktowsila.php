...
require_once "database/db-connect.php";
		...
		$input_pkt_sila = $_GET['input-pkt-sila'];
		 
		 if(empty($input_pkt_sila)){
			 ...
			 exit();
						  ...
						  $sql="SELECT pkt, sila, respekt, energia FROM uzytkownicy WHERE id='$user_id'";
						  ...
						  $nowepkt = $row['pkt'] - $input_pkt_sila;
			 if($nowepkt<0){
			 ...
			 header ('Location: ../game-crims-city.php?link=character');
						  ...
						  $baza_pkt_sila = $row['sila'];
						  $respekt = $row['respekt'];
						  $nowepktsila = $row['sila'] + $input_pkt_sila;
						
						  $sql1="UPDATE `uzytkownicy` SET `pkt`= '$nowepkt' WHERE id='$user_id'";
						  $sql2="UPDATE `uzytkownicy` SET `sila`= '$nowepktsila' WHERE id='$user_id'";
						  $nowy_respekt = $respekt + (60 * $input_pkt_sila);
